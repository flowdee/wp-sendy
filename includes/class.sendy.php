...
if ( ! class_exists('SFWP_Sendy_API') ) {

    class SFWP_Sendy_API
    {
        protected $options;
        ...
        public function __construct( $installation_url = null, $api_key = null )
        {
            $this->options = sfwp_get_options();

            //error checking
            $installation_url = ( ! empty( $installation_url ) ) ? $installation_url : $this->options['api_url'];
            $api_key = ( ! empty( $api_key ) ) ? $api_key : $this->options['api_key'];
        ...
        private function buildAndSend($type, array $values)
        {
            ...
            $args = wp_parse_args( $values, $return_options );

            sfwp_debug( $args, 'API args' );
